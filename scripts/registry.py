...
Generated artifacts:
    ...
    REGISTRY.md                human browse index (one summary line per bot)
...
REGISTRY_MD = REPO_ROOT / "REGISTRY.md"
...
def render_md(entries: list[dict]) -> str:
    """Human browse index: one summary line per bot (full details in zone JSON)."""
    lines = [
        "# Bot Registry",
        "",
        "_Auto-generated by `scripts/registry.py`. Do not edit by hand._",
        "_Full per-bot detail lives in each zone's `registry.json`; "
        "global search uses `index.json`._",
        "",
        f"**{len(entries)} bot(s)** across "
        f"**{len({e['world'] for e in entries})} world(s)**.",
        "",
    ]
    current_world = current_zone = None
    for e in entries:
        if e["world"] != current_world:
            current_world = e["world"]
            current_zone = None
            lines += [f"## {current_world}", ""]
        if e["zone"] != current_zone:
            current_zone = e["zone"]
            lines += [f"### `{current_zone}`", ""]
        meta = []
        if e["format"]:
            meta.append(f"`{e['format']}`")
        if e["author"]:
            meta.append(f"by {e['author']}")
        if e["clients"]:
            meta.append(f"clients `{e['clients']}`")
        suffix = f" — {' · '.join(meta)}" if meta else ""
        summary = summary_line(e["description"])
        if summary:
            suffix += f"<br>{summary}"
        lines.append(f"- [{e['name']}]({e['path']}){suffix}")
    lines.append("")
    return "\n".join(lines)
...
def stale_files(outputs: dict[Path, str]) -> set[Path]:
    ...
    legacy_root = REPO_ROOT / "registry.json"  # from the old single-file layout
...
def cmd_build(check: bool) -> int:
    ...
    print(
        f"Wrote index.json, REGISTRY.md, and {zone_files} per-zone "
